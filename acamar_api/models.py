...
class Course(ModelMeta, SafeDeleteModel):
    ...
    meta_description = models.CharField(
        verbose_name="Meta description", max_length=254, blank=True
    )
    meta_keywords = models.CharField(
        verbose_name="Meta keywords", max_length=254, blank=True
    )
    ...
    def get_meta_description(self):
        return (
            self.meta_description if self.meta_description else self.short_description
        )
...
class CourseTermItem(models.Model):
    ...
    def time(self):
        if self.end_time:
            return "{}-{}".format(
                dateformat(self.start_time, "G.i"), dateformat(self.end_time, "G.i")
            )
...
class Position(ModelMeta, models.Model):
    ...
    _user_image_url = models.URLField(
        max_length=254, blank=True, db_column="user_image_url"
    )
    user_phone = models.CharField(max_length=254, blank=True)
    user_position = models.CharField(max_length=254, blank=True)
    technologies = models.ManyToManyField(
        "acamar_api.PositionTechnology", related_name="positions", blank=True
    )
    pacts = models.ManyToManyField(
        "acamar_api.PositionPact", related_name="positions", blank=True
    )
    ...
    def get_absolute_url(self, language=None):
        ...
        if language != self.lang:
            translated = Position.objects.filter(
                internal_id=self.internal_id, lang=language
            ).first()
            if translated:
    ...
    def recruiter(self):
        return Recruiter.objects.filter(
            first_name=self.user_first_name, second_name=self.user_second_name
        ).first()
    ...
    def _get_unique_slug(self):
        if self.title:
            ...
            num = 1
            while (
                Position.objects.filter(lang=self.lang, slug=unique_slug)
                .exclude(id=self.id)
                .exists()
            ):
    ...
    def get_meta_description(self):
        return _(
            "Příjmi nový IT projekt jako svoji výzvu. Získej odměnu, zkušenosti a posuň se dál! Pracuj s českými i mezinárodními týmy."
        )
...
class CourseEnroll(models.Model):
    ...
    courses = models.ManyToManyField(
        "acamar_api.CourseTerm", related_name="enrolled", verbose_name="Course terms"
    )
    expectations = models.TextField(verbose_name="Expectations", blank=True)
    cv = models.FileField(
        verbose_name="CV", upload_to=cv_upload_to, null=True, blank=True
    )
    ...
    def course_list(self):
        return ",".join(
            Course.objects.filter(terms__in=self.courses.all()).values_list(
                "title", flat=True
            )
        )
...
class PositionApply(models.Model):
    ...
    position_user_name = models.CharField(
        verbose_name="Position - person", max_length=254
    )
    position_user_email = models.EmailField(
        verbose_name="Position - person email", max_length=254
    )
    ...
    def save(
        ...
        if self.position:
            ...
            self.position_user_name = "{} {}".format(
                self.position.user_first_name, self.position.user_second_name
            )
            self.position_user_email = self.position.user_email
        super(PositionApply, self).save(

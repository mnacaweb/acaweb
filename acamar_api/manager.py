...
class AcamarPositionManager:
    url = "https://www.acamar.cz/api_pozice.php"
    cache_prefix = "acamar_position_{}"
    cache_duration = 300

    @classmethod
    def _request(cls, lng):
        resp = requests.get(url=cls.url, params={"token": "ace247464bfa9076dada655abea751c6", "lng": lng})
        if resp.status_code == 200:
            return resp.json()
        ...
        for language, _ in settings.LANGUAGES:
            with translation.override(language):
                ...
                for position in cls.get_positions(language):
                    ...
                    obj, _ = Position.objects.update_or_create(id=position["id"], defaults={
                        "lang": True,
